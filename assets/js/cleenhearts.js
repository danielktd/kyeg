(function($) {
    ...
    counters.forEach((counter) => {
      ...
      const updateCounter = () => {
        ...
        const increment = target / 200; // Adjust the divisor to control the speed
    
        if (count < target) {
          count = Math.ceil(count + increment); // Increment count
          counter.innerText = count < target ? count : target; // Update the counter text
          setTimeout(updateCounter, 100); // Adjust the timeout for the desired speed
        }
      };
      updateCounter();
    });
